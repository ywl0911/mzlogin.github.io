...
$$p(x)=\sum_{k=1}^2p(k)p(x\vert k)=\pi_1N(x\vert \mu_1,\sigma_1)+\pi_2N(x\vert \mu_2,\sigma_2)$$
...
$${Q_i(z^{(i)})}=\frac{p(x^{(i)},z^{(i)};\theta)}{p(x^{(i)};\theta)}=p(z^{(i)}\vert x^{(i)};\theta)$$

当$${Q_i(z^{(i)})}=p(z^{(i)}\vert x^{(i)};\theta)$$，$$J(Q,\theta)$$会增加到与$$L(\theta)$$相等。下一步在$${Q_i(z^{(i)})}=p(z^{(i)}\vert x^{(i)};\theta)$$的情况下，调整参数$$\theta$$来求$$J(Q,\theta)$$的极大值进一步极大化$$L(\theta)$$。
...
&emsp;&emsp;  $${Q_i(z^{(i)})}:=p(z^{(i)}\vert x^{(i)};\theta)$$
...
$$\geq \sum_{i=1}^{n} \sum_{z^{(i)}}Q_i(z^{(i)})\ln \frac {p(x^{(i)},z^{(i)};\theta^{(t)})}{Q_i(z^{(i)})}=L(\theta^{(t)})$$
